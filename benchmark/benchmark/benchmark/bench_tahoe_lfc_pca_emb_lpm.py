"""
Benchmark script for tahoe LFC on LPM-restricted compounds.

Restricts compounds to the INTERSECTION of:
  - PCA pkl compounds (dose-specific from tahoe_PCA_emb.pkl)
  - LPM-available compounds (from tahoe_sci_op3_updated.pkl)

This gives a reduced compound set (88 for dose 3.33, 63 for dose 10) so all
embeddings including LPM are evaluated on the same compounds.

Supports all embedding types: PCA pkl, h5ad (MiniMol, AIDOcell, chatgpt),
sci pkl (LPM_emb, ECFP:2), random, pca, and baselines.
...
Usage:
    ...
    python benchmark/benchmark/bench_tahoe_lfc_pca_emb_lpm.py --config-name config_tahoe_lfc_pca_emb_lpm_dose3 --multirun hydra/launcher=joblib hydra.launcher.n_jobs=4

    # Dose 10 (CVCL_0023)
    python benchmark/benchmark/bench_tahoe_lfc_pca_emb_lpm.py --config-name config_tahoe_lfc_pca_emb_lpm_dose10 --multirun hydra/launcher=joblib hydra.launcher.n_jobs=4
"""
...
def get_valid_cids(dose):
    """Return set of pubchem_cids: intersection of PCA pkl (dose-specific) and LPM availability."""
    # PCA pkl compounds for this dose
    pca_df = pd.read_pickle(PKL_PATH)
    pca_cids = set(pca_df[pca_df["l1000_pert_dose_uM"] == dose]["pubchem_cid"].astype(int).unique())

    # LPM-available compounds
    sci_df = pd.read_pickle(SCI_PKL_PATH)
    tahoe_sci = sci_df[sci_df["dataset"] == "tahoe"]
    lpm_cids = set(
        tahoe_sci[tahoe_sci["LPM_emb"].notna()]
        .drop_duplicates(subset="pubchem_cid", keep="first")["pubchem_cid"]
        .astype(int)
    )

    return pca_cids & lpm_cids
...
def get_submission_dir(dose):
    ...
    return SUBMISSION_DIR / f"expression/pert_prediction_tahoe_regression_pca_emb_lpm_dose_{dose_str}"
...
@hydra.main(version_base=None, config_path="config/tahoe", config_name="config_tahoe_lfc_pca_emb_lpm_dose3")
def main(cfg: DictConfig) -> None:
    ...
    # Restrict to valid compounds (PCA ∩ LPM)
    ...
    description += f"Restricted to {len(valid_cids)} compounds at dose {dose} uM (PCA ∩ LPM)\n"
