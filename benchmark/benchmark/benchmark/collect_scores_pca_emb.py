"""
Collect PCA embedding JSON submission files into CSV score files.

Produces 4 output CSVs (one per task/dose combination):
  - results/scores/tahoe_lfc_pca_emb_dose3.csv
  - results/scores/tahoe_lfc_pca_emb_dose10.csv
  - results/scores/tahoe_deg_pca_emb_dose3.csv
  - results/scores/tahoe_deg_pca_emb_dose10.csv

Usage:
    python collect_scores_pca_emb.py
"""
...
def normalize_name_lfc(raw_name):
    ...
    context mean baseline -> context mean baseline
    no change baseline -> no change baseline
    knn_baseline_PCA.logFC_v1_64 -> knn_baseline_PCA.logFC_v1_64
    """
    # LFC baselines already have the right name format
    return raw_name
...
def main():
    print("=" * 60)
    print("Collecting LFC dose 3.33333")
    print("=" * 60)
    collect_lfc(
        SUBMISSION_DIR / "expression" / "pert_prediction_tahoe_regression_pca_emb_dose_3_33333",
        REPO_ROOT / "results" / "scores" / "tahoe_lfc_pca_emb_dose3.csv",
    )

    print("\n" + "=" * 60)
    print("Collecting LFC dose 10")
    print("=" * 60)
    collect_lfc(
        SUBMISSION_DIR / "expression" / "pert_prediction_tahoe_regression_pca_emb_dose_10_0",
        REPO_ROOT / "results" / "scores" / "tahoe_lfc_pca_emb_dose10.csv",
    )

    print("\n" + "=" * 60)
    print("Collecting DEG dose 3.33333")
    print("=" * 60)
    collect_deg(
        SUBMISSION_DIR / "expression" / "pert_prediction_tahoe_deg_pca_emb_dose_3_33333",
        REPO_ROOT / "results" / "scores" / "tahoe_deg_pca_emb_dose3.csv",
    )

    print("\n" + "=" * 60)
    print("Collecting DEG dose 10")
    print("=" * 60)
    collect_deg(
        SUBMISSION_DIR / "expression" / "pert_prediction_tahoe_deg_pca_emb_dose_10_0",
        REPO_ROOT / "results" / "scores" / "tahoe_deg_pca_emb_dose10.csv",
    )
